...
describe("readPackageDeps - classic strategy", () => {
    ...
    it("returns 'null' if the package.json does not exist over there", () => {
        process.chdir('test/extract/resolve/readPackageDeps/fixtures/no-package-json-here');
        expect(
            readPackageDeps(path.parse(process.cwd()).root)
        ).to.equal(null);
    });
...
describe("readPackageDeps - combined dependencies strategy", () => {
    ...
    it("returns the deps if the package.json exists in the baseDir", () => {
        process.chdir('test/extract/resolve/readPackageDeps/fixtures/package-json-in-here');
        expect(
            readPackageDeps(
                process.cwd(),
                process.cwd(),
    ...
    it("returns the combined deps if there's a package.json in both base and sub package", () => {
        process.chdir('test/extract/resolve/readPackageDeps/fixtures/two-level-package-jsons');
        expect(
            readPackageDeps(
    ...
    it("returns the combined deps if there's a package.json in both base and sub package - subdir of sub", () => {
        ...
        expect(
            readPackageDeps(
                path.join(process.cwd(), 'packages', 'subthing', 'src', 'somefunctionality'),
                process.cwd(),
                true
            )
        ).to.deep.equal(
