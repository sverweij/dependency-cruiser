...
describe("pathToPosix on posix", () => {
    it("leaves win32 style paths alone: \"\"", () => {
        expect(
            pathToPosix("", path.posix)
        ).to.equal("");
    });

    it("leaves win32 style paths alone: \\root\\sub\\file.txt", () => {
        expect(
            pathToPosix("\\root\\sub\\file.txt", path.posix)
        ).to.equal("\\root\\sub\\file.txt");
    });

    it("leaves win32 style paths alone: C:\\root\\sub\\file.txt", () => {
        expect(
            pathToPosix("C:\\root\\sub\\file.txt", path.posix)
        ).to.equal("C:\\root\\sub\\file.txt");
    });
