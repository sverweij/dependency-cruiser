...
describe("flatten typescript config - simple config scenarios", () => {
    ...
    it("throws when a config file is passed that does not contain valid json", () => {
        expect(
            () => parseTSConfig(path.join(__dirname, "./fixtures/typescriptconfig/tsconfig.invalid.json"))
        ).to.throw();
    });
...
describe("flatten typescript config - 'extend' config scenarios", () => {
    ...
    it("throws when a config file is passed that has a circular reference", () => {
        expect(
            () => parseTSConfig(path.join(__dirname, "./fixtures/typescriptconfig/tsconfig.circular.json"))
        ).to.throw("error TS18000: Circularity detected while resolving configuration");
    });
